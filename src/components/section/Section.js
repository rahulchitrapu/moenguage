...
import SelectCompo from "../molecules/materialUI/SelectCompo";

export default function Section({ remove, id, setActive, getData,defindClass }) {
  const baseurl =
    "https://ancillary-prod.smytten.com/smyttenresearch/targetingproperties/";
  ...
  useEffect(() => {
    axios.get(`${baseurl}6352373946f76519425c02da`).then((res) => {
      setOptions(res.data);
    });
  }, []);
  ...
  useEffect(() => {
    if (selectedID !== "") {
      axios.get(`${baseurl}${selectedID}`).then((res) => {
  ...
  function controlValueChanger(control, controlValue) {
    switch (control) {
        case "IS_BETWEEN":
            return controlValue ;
    ...
    if (controlValue && defindClass==="") {
      ...
      let obj = {
        ...
        condition: {
          ...
          value: controlValueChanger(control, controlValue),
        },
      };
      getData(obj, id);
    }
  ...
  function buttonClickHandler(name) {
    ...
    setControl("");
    setControlValue();
    axios.get(baseurl + name.id).then((res) => {
      setOptions(res.data);
    });
  }
          ...
          {names.map((name) => {
            ...
            return (
              <button
                key={name.id}
                className={
                  buttonState === name.value ? "activeButton" : "button"
                }
                onClick={() => {
